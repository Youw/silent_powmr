...
constexpr TickType_t kReplyTimeout = pdMS_TO_TICKS(300);  // total wait for a reply
constexpr TickType_t kIdleGap = pdMS_TO_TICKS(20);        // end-of-frame silence
}  // namespace
...
size_t UartTransport::transfer(const uint8_t* req, size_t req_len, uint8_t* resp,
  ...
  size_t got = 0;
  TickType_t waited = 0;
  // Read until an idle gap (frame complete) or the overall timeout elapses.
  while (got < resp_cap && waited < kReplyTimeout) {
    const int n = uart_read_bytes(port_, resp + got, resp_cap - got, kIdleGap);
    if (n > 0) {
      got += static_cast<size_t>(n);
    } else {
      if (got > 0) break;  // had data, then silence -> end of frame
      waited += kIdleGap;
    }
  }
  return got;
